...
public class Door : Interactable
{
    ...
    void Start()
    {
        ...
        if (roomManager == null)
        {
            roomManager = GameObject.FindAnyObjectByType<RoomStateManager>();
        }
    ...
    public override void Interact()
    {
        ...
        {
            hasOpened = true;
            Debug.Log("Door: Key validated! Opening door into pure white light.");
    ...
    private IEnumerator ShowWarningRoutine(string message)
    {
        ...
        onScreenWarningUI.fontSize = 19f;
        onScreenWarningUI.fontStyle = FontStyles.Normal;
        onScreenWarningUI.color = Color.white;
    ...
    private IEnumerator OpenDoorAndEscape()
    {
        ...
        // Set camera clearFlags to SolidColor with pure white so any view through the opening door renders as pure white celestial light!
        ...
        // Trigger Step 5: Doorway Escape sequence in RoomStateManager immediately as the door opens
        if (roomManager != null)
        {
            roomManager.TriggerDoorwayEscape(openDuration, transform);
        }
        else
        {
            RoomStateManager rm = GameObject.FindAnyObjectByType<RoomStateManager>();
            if (rm != null) rm.TriggerDoorwayEscape(openDuration, transform);
        }
